...
3. Get your voice_id from https://elevenlabs.io/app/voice-lab (or use a premade voice ID)
4. Run: python generate_tts_elevenlabs.py YOUR_API_KEY YOUR_VOICE_ID

Pricing: ~8,233 characters total
- Free tier: 10,000 chars/month (covers 1 generation)
...
if len(sys.argv) < 3:
    print("Usage: python generate_tts_elevenlabs.py YOUR_API_KEY YOUR_VOICE_ID")
    print("\nTo find your voice ID:")
    print("1. Go to https://elevenlabs.io/app/voice-lab")
    print("2. Click on your custom voice")
    print("3. Copy the Voice ID from the settings")
    sys.exit(1)
...
# Voice settings from your custom voice configuration
...
def generate_clip(client, text, filepath):
    ...
    try:
        # Generate audio using the new API with English language override
